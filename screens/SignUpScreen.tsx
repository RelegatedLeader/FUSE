...
export default function SignUpScreen({ navigation }) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TextInput style={styles.input} placeholder="Current Occupation" value={occupation} onChangeText={setOccupation} />
      <TextInput style={styles.input} placeholder="Career Aspiration" value={careerAspiration} onChangeText={setCareerAspiration} />

      <Text style={styles.section}>Personality Traits (0-100%)</Text>
      <Text>Extroversion: {extroversion}%</Text>
      <TextInput style={styles.input} placeholder="0-100" keyboardType="numeric" value={extroversion.toString()} onChangeText={(v) => setExtroversion(parseInt(v) || 0)} />
      <Text>Openness: {openness}%</Text>
      <TextInput style={styles.input} placeholder="0-100" keyboardType="numeric" value={openness.toString()} onChangeText={(v) => setOpenness(parseInt(v) || 0)} />
      <Text>Conscientiousness: {conscientiousness}%</Text>
      <TextInput style={styles.input} placeholder="0-100" keyboardType="numeric" value={conscientiousness.toString()} onChangeText={(v) => setConscientiousness(parseInt(v) || 0)} />
      <Text>Agreeableness: {agreeableness}%</Text>
      <TextInput style={styles.input} placeholder="0-100" keyboardType="numeric" value={agreeableness.toString()} onChangeText={(v) => setAgreeableness(parseInt(v) || 0)} />
      <Text>Neuroticism: {neuroticism}%</Text>
      <TextInput style={styles.input} placeholder="0-100" keyboardType="numeric" value={neuroticism.toString()} onChangeText={(v) => setNeuroticism(parseInt(v) || 0)} />

      <Text style={styles.section}>MBTI</Text>
      <Button title="Take Official MBTI Test" onPress={handleMbtiLink} />
      <TextInput style={styles.input} placeholder="Enter your MBTI (e.g., ENFP)" value={mbti} onChangeText={setMbti} />

      <Text style={styles.section}>Open-Ended Questions</Text>
      <TextInput style={[styles.input, { height: 100 }]} placeholder="Tell us about yourself..." multiline value={openEnded} onChangeText={setOpenEnded} />
...
const styles = StyleSheet.create({
  ...
  section: { fontSize: 18, marginTop: 20, marginBottom: 10, fontWeight: 'bold' },
